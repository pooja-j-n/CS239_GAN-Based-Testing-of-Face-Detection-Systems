...
def print_result(epoch, iters, accuracy, D_loss, G_loss, gan_loss, cyc_loss, cls_loss):
    ...
    print('D loss : %f' % (D_loss.item()))      #---- D_loss.data[0]
    print('G loss : %f' % (G_loss.item()))
    print('  * gan loss : %f' % (gan_loss.item()))
    print('  * cyc loss : %f' % (cyc_loss.item()))
    print('  * cls loss : %f' % (cls_loss.item()))
    print('===============================================\n')
...
def get_cls_accuracy(score, label):
    ...
    dtype = type(label)
    if(torch.cuda.is_available()):     #-----added extra condition
        correct = torch.sum(pred == label.cuda())
    else:
        correct = torch.sum(pred == label.cpu())
